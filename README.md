...
> **A lightweight frequency-decomposed linear model for long-term time-series forecasting — best overall accuracy across 7 datasets at long lookback, beating iTransformer (ICLR 2024) by 4.3% and TimesNet (ICLR 2023) by 23.2%, at 13× less memory and 18× faster training, on a single 4 GB laptop GPU.**
...
**Highlights**
- 🏆 **Best overall accuracy of 9 models** at long lookback ($L=336$, 7 datasets): avg MSE **0.3177** vs RLinear **0.3199**, iTransformer **0.3318** (paired −4.3%), TimesNet **0.4134** (−23.2%); on the 5 standard benchmarks −6.9% vs iTransformer, −20.8% vs TimesNet, −9.6% vs PatchTST. All significant (paired Wilcoxon $p<10^{-4}$).
- ⚡ **13× less memory, 18× faster** than TimesNet; runs on a single 4 GB laptop GPU across 7 datasets including Traffic (862 channels).
- 🔁 **A-RevIN** engages under non-stationarity (ILI: up to ~5% MSE reduction; confirmed on a controlled synthetic drift sweep and distribution-shift stress test) and reduces exactly to RevIN otherwise.
...
.venv\Scripts\python.exe scripts\download_data.py --with-ecl            # + Electricity (321 ch)
.venv\Scripts\python.exe scripts\download_data.py --traffic             # + Traffic (862 ch)
```

Canonical wide-format LTSF CSVs (column `date` + channels), byte-identical to the
Autoformer/DLinear release, downloaded from public mirrors. SHA-256 of each file is
written to `data/checksums.txt`. Shapes: ETTh 17420 rows, ETTm 69680, Weather 52696,
Electricity 26304, Traffic 17544.
