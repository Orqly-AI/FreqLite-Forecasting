...
tokenizes the series into subseries patches and applies channel-independent
Transformer attention, achieving strong performance on standard benchmarks;
iTransformer \citep{liu2024itransformer} (ICLR 2024) inverts the tokenization so
each variate becomes a token and attention models cross-channel dependencies,
yielding a compact memory footprint compared to PatchTST but requiring all-variate
mixing on each forward pass; TimesNet \citep{wu2023timesnet} (ICLR 2023) detects
dominant periods via FFT and applies 2D Inception convolutions within each period,
combining temporal and channel structure at substantial memory cost
($\sim$1\,GB at typical settings). These three represent the recent SOTA we
include as baselines---the central empirical question being: can a linear-scale
model recover their accuracy at a fraction of the cost? Our results
(Section~\ref{sec:results:main}) show it can, beating iTransformer by $4.3\%$
and TimesNet by $23.2\%$ in paired per-cell MSE over seven datasets at
$L{=}336$, while using $13\times$ less memory than TimesNet. We adopt PatchTST's channel-independent treatment but reject patching
and attention.
