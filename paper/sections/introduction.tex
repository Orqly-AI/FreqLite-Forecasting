...
\emph{modeled} rather than thrown away. Empirically, \method\ attains the best
overall average MSE and the most per-setting wins of any of the nine models we
evaluate at long lookback ($L{=}336$) across seven datasets, and---most
notably---\emph{surpasses all three Transformer baselines}: it beats
iTransformer \citep{liu2024itransformer} (ICLR 2024) by $\mathbf{4.3\%}$ and
TimesNet \citep{wu2023timesnet} (ICLR 2023) by $\mathbf{23.2\%}$ in paired
per-cell MSE over the full seven-dataset grid ($6.9\%$ and $20.8\%$ on the five
standard benchmarks), while using $2.5\times$ fewer parameters, $13\times$ less
peak GPU memory, and $18\times$ less training time than TimesNet---all on a
single 4\,GB laptop GPU.
...
\begin{itemize}
  \item \textbf{\method.} An ultra-lightweight frequency-decomposed linear
        ...
        heads---that at long lookback ($L{=}336$) attains the best overall
        average MSE across seven datasets and \textbf{outperforms iTransformer
        (ICLR 2024) by 4.3\% and TimesNet (ICLR 2023) by 23.2\%} in paired
        per-cell test MSE, while using $13\times$ less GPU memory and
        $18\times$ less training time than TimesNet, on a single 4\,GB laptop GPU.
        Results are statistically significant under paired Wilcoxon tests
        ($p \approx 2\times10^{-5}$ vs.\ iTransformer, $p \approx 2\times10^{-15}$
        vs.\ TimesNet).
  \item \textbf{A-RevIN.} A regime-adaptive reversible normalization that
        ...
        dataset (ILI) and a distribution-shift stress test (Section~\ref{sec:results:shift})---while
        reducing to RevIN on low-stationarity data with no harm.
  \item \textbf{A reproducible, comprehensive accuracy--efficiency study} across
        seven datasets (including high-variate Electricity with 321 channels and
        Traffic with 862 channels) on commodity hardware, with ablations isolating
        each component and a controlled distribution-shift stress test.
\end{itemize}
